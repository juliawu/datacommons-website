...
/** Page rendering for the experimental place pages */
...
const OVERVIEW_TILE_DIV_ID = "overview-tile-container";
const GOOGLE_MAP_DIV_ID = "google-map-container";
...
function renderPage(): void {
  const dcid = document.getElementById("place-dcid").dataset.dcid;
  ...
  // Render google map in manually created overview tile, if a placeholder
  // for a google map is provided in the template
  if (document.getElementById(GOOGLE_MAP_DIV_ID)) {
    ReactDOM.render(
      React.createElement(GoogleMap, {
        dcid,
      }),
      document.getElementById(GOOGLE_MAP_DIV_ID)
    );
  }
}
